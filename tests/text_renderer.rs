...
fn dynamic_text_updates_vertices_and_respects_capacity() {
    ...
    let mut ctx = setup_ctx();
    let mut res = ResourceManager::default();
    let info = DynamicTextCreateInfo { max_chars: 8, text: "hi", scale: 16.0, pos: [0.0, 0.0], screen_dim: [1280.0, 1024.0], key: "dtex" };
...
fn dynamic_text_update_empty_string_resets_counts() {
    ...
    let mut ctx = setup_ctx();
    let mut res = ResourceManager::default();
    let info = DynamicTextCreateInfo { max_chars: 8, text: "hello", scale: 16.0, pos: [0.0, 0.0], screen_dim: [1280.0, 1024.0], key: "emt" };
    let mut dt = DynamicText::new(&mut ctx, &text, &mut res, info).unwrap();
...
fn dynamic_text_update_over_capacity_panics() {
    ...
    let mut ctx = setup_ctx();
    let mut res = ResourceManager::default();
    let info = DynamicTextCreateInfo { max_chars: 2, text: "hi", scale: 16.0, pos: [0.0, 0.0], screen_dim: [1280.0, 1024.0], key: "ovr" };
    let mut dt = DynamicText::new(&mut ctx, &text, &mut res, info).unwrap();
